...
export async function taskAnalysis(token) {
    try {
        const response = await axios.get(`${process.env.EXPO_PUBLIC_BACKEND_URL}/tasks/dashboard`, { headers: { 'Authorization': `Bearer ${token}`, "ngrok-skip-browser-warning": "69420" } });
        if (response.status === 200) {
...
export async function markAsCompleted(token, taskId) {
    try {
        const response = await axios.post(`${process.env.EXPO_PUBLIC_BACKEND_URL}/tasks/${taskId}/complete`, {}, { headers: { 'Authorization': `Bearer ${token}`, "ngrok-skip-browser-warning": "69420" } });
        if (response.status === 200) {
            return { success: true, message: response.data.message }
        } else {
            return { success: false }
