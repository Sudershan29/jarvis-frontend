...
const Event = ({ events, heading, isDate }) => {
    ...
    return (
        <Box>
            {!isDate && <Typography className={classes.heading}>{heading}</Typography>}
            {isDate && <Typography className={classes.heading}>{heading?.day}, {heading?.date + " " + heading?.month}</Typography>}
            {events.length === 0 ?
